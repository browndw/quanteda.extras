...
**Deviation of proportions** (**DP**) is a dispersion statistic proposed by Gries (2010), which measures dispersion on a scale of 0 to 1 such that tokens with DP close to zero are **more** dispersed while those closer to 1 are **less** dispersed.
...
The **dispersions_token( )** function calculates the dispersion measures for a single token. It returns a named list with all of the dispersion measures discussed by [S.T. Gries](https://www.stgries.info/research/dispersion/links.html) and the function is baesed on a script he originally authored.
