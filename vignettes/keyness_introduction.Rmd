...
Next, we'll use [**quanteda**](http://quanteda.io/) to create a corpus and tokenize the data. Many of the functions in  **quanteda.extras** require either a **quanteda tokens** object or a **quanteda dfm** object.

```{r corpus, message = FALSE, error=FALSE, warning=FALSE}
sc <- corpus(sc) %>% tokens(what="fastestword", remove_numbers=TRUE)
```
